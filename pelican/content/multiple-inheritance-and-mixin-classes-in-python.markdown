...
Delegation was introduced in OOP as a way to reduce code duplication. When an object needs a specific feature it just delegates it to another class (either explicitly of implicitly), so the code is written just once. For example, in the classic case of elements of a GUI we might have the following hierarchy

``` text
    graphical interactive element
          (resize, click)
                 ^
                 |
                 |
...
Let's continue the example of the GUI elements, and consider a `text box`, which is an element that we want to resize, but cannot be clicked. If we create a hierarchy like this
...
we end up putting the interactive features in an object that shouldn't have them. The standard solution to this problem is that of increasing the depth of the inheritance hierarchy and to derive from the new simpler ancestor.
...
However, this approach stops being viable as soon as an object needs to inherit from a given class but not from the parent of that class. For example, an element that has to be interactive but not resizable, like a `menu entry`. If we want to keep using inheritance, the only solution at this point is to duplicate the code that implements the resize feature (or the code that implements the interactive nature of the element), and create two different class hierarchies.

``` text
  graphical element        graphical interactive element
      (resize)                    (resize, click) <----------- CODE DUPLICATION
          ^                              ^
          |                              |
          |                              |
          |                              |
...
Unfortunately, chances are that this is only the first of many compromises we will have to accept to keep the system in a stable state, if we can't change our approach.
...
The above problem can them be solved having `button` inherit from both the class that provides the resize feature and from the one that implements the interactive nature. 

``` text
  graphical element             interactive element
      (resize)                        (click)
          ^ ^                            ^
          | |                            |
          | +--------------------------+ |
...
Unfortunately things are not that simple. First of all, we face the issue every microservice-oriented architecture faces, that is the risk of going from god objects (the extreme monolithic architecture) to almost empty objects (the extreme distributed approach), burdening the programmer with too a fine-grained control that eventually results in a system where relationships between objects are so complicated that it becomes impossible to grasp the effect of a change in the code.

There is a more immediate problem in multiple inheritance, though. As it happens with the natural inheritance, parents can provide the same "genetic trait" in two different flavours, but the resulting individual will have only one. Genetically, we are not exactly half our father and half our mother, as the genes of one or the other have different strengths, but leaving aside human genetics (which is incredibly more complicated than programming) and going back to OOP, we face a problem when an object inherits from two other objects that provide the same feature.
...
Parents can have different signatures of the common method, for example
...
The problem can be extended further, introducing a common ancestor above `Parent1` and `Parent2`.
...
As you can see, we already have problems with multiple parents, and a common ancestor just makes them worse. The ancestor class can clearly be at any point of the inheritance tree, the important part is that it is shared between `Parent1` and `Parent2`. This is the so called diamond problem, as the inheritance graph has the shape of a diamond
...
Does all this make multiple inheritance useless?

Not at all! There are solutions to this problem, as we will see shortly, but this further level of intricacy makes multiple inheritance something that doesn't fit easily in a design, and has to be implemented carefully to avoid subtle bugs. Remember that inheritance is an automatic delegation mechanism, as this makes what happens in the code less evident. For these reasons, multiple inheritance is often depicted as scary and convoluted, and usually given some space only in the advanced OOP courses, at least in the Python world. I believe every Python programmer, instead, should familiarise with it and learn how to take advantage from it.
...
As we see with the `flush` method, Python doesn't change it's behaviour when it comes to method overriding with multiple parents. The first implementation of a method with that name is executed, and the parent's implementation is not automatically called. As in the case of standard inheritance, then, it's up to us to design classes with matching method signatures.
...
Finally, let's get to the original source of inspiration for this post: the Django code base. I will show you here how the Django programmers used multiple inheritance and mixin classes to promote code reuse, and you will now hopefully grasp all the reasons behind them.
...
* Mixins are implemented in Python using multiple inheritance: they have a great expressive power but require careful design.
